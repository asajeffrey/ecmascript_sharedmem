...
The ─hb→ relation allows events to be executed simultaneously:
*d* ←hb→ *e* whenever *d* and *e* events from the same atomic operation,
and must be executed together.
...
* *per-byte sequential consistency*: atomic read and
  write events are sequentially consistant, and
...
The requriements of per-byte sequential consistency and
